...
class ECAMP(nn.Module):
    ...
    def mask_2_pixel(self, mask, column, row):
        ...
        # ----- ablation for Super resolution -----

        for i in range(mask.shape[0]):
            super_mask[i, column[i]:column[i]+12, row[i]:row[i]+12] = 1

        # ----- end ablation for Super resolution -----
        # ipdb.set_trace()
    ...
    def forward_report_decoder(self, latent, ids_keep, caption_ids, attention_mask, token_type_ids):
        latent = self.bert_mlp(latent)
        gap_token = latent[:, 1:, :].mean(dim=1)
        gap_token = gap_token.unsqueeze(1)
        latent = latent[:, 1:, :]
        outputs = self.bert_encoder(latent, gap_token, caption_ids, attention_mask, token_type_ids)
        return outputs
    ...
    def forward(self, imgs, text_ids, attention_mask, type_ids, mask_ratio=0):
        imgs = imgs.cuda()
        ids = text_ids.cuda()
        ...
        cross_attn = self.forward_report_decoder(latent, ids_keep, ids, attention_mask, type_ids)
        return cross_attn
